# SVM classification based on doc2vec
...
def binary_label(y):
    """ Convert a score between 1 and 5 to + and - """
    if y > 3:
        return "+"
    else:
        return "-"
...
output_dir = "{0}/output-svm".format(dir_path)
...
for test_fold in folds:
    ...
    clf_multi = svm.SVC()
    clf_multi.fit(train_vecs, train_labels)

    clf_bin = svm.SVC()
    # 4 and 5 become 1; 1, 2 and 3 become 0
    clf_bin.fit(train_vecs, [binary_label(y) for y in train_labels])
    print("Done training")

    actual_labels_multi = labels[test_fold-1]
    actual_labels_bin = [binary_label(y) for y in actual_labels_multi]
    ...
    for i in range(0, max_round):
        ...
        print(clf_multi.predict(test_vecs[start_idx:end_idx]))
        print(clf_bin.predict(test_vecs[start_idx:end_idx]))

        predict_labels_multi = predict_labels_multi + clf_multi.predict(test_vecs[start_idx:end_idx]).tolist()
        predict_labels_bin = predict_labels_bin + clf_bin.predict(test_vecs[start_idx:end_idx]).tolist()

        print("Done {0} / {1}".format(i+1, max_round))

    del clf_multi, clf_bin
    ...
    for i in range(0, len(actual_labels_multi)):
        print(i)
        actual_label_bin = actual_labels_bin[i]
        predict_label_bin = predict_labels_bin[i]

        if (actual_label_bin == "+") and (predict_label_bin == "+"):
            tp += 1
        elif (actual_label_bin == "+") and (predict_label_bin == "-"):
            fn += 1
        elif (actual_label_bin == "-") and (predict_label_bin == "+"):
            fp += 1
        else:
        ...
        outf.write("{0}\t{1}\t{2}\t{3}\n".format(
            actual_labels_multi[i],
            predict_labels_multi[i],
            actual_label_bin,
            predict_label_bin))
    outf.close()
